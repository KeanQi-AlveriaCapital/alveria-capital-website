...
function AnimatedNavLink({ href, children, className = "", style = {}, isDarkMode = false }: {
  href: string;
...
function AnimatedMobileLink({ href, children, className = "", style = {}, isDarkMode = false }: {
  href: string;
  children: React.ReactNode;
  className?: string;
  style?: React.CSSProperties;
...
function LanguageToggle({ isDarkMode = false }: { isDarkMode?: boolean }) {
  ...
  return (
    <ToggleGroup 
      type="single" 
      value={language} 
      onValueChange={(value: string) => value && setLanguage(value as "EN" | "中文")}
...
function MobileLanguageDropdown({ isDarkMode = false }: { isDarkMode?: boolean }) {
  const { language, setLanguage } = useLanguage();
      ...
      {isOpen && (
        ...
        >
          <button
            ...
            className={`w-full px-4 py-3 text-left text-p3 font-medium transition-all duration-300 hover:bg-[var(--outer-space)] ${language === 'EN' ? 'bg-[var(--dawn)]' : 'bg-[var(--mercury)]/8'}`}
            style={{ color: language === 'EN' ? activeTextColor : inactiveTextColor }}
          >
            EN
          </button>
          <button
            ...
            className={`w-full px-4 py-3 text-left text-p3 font-medium transition-all duration-300 hover:bg-[var(--outer-space)] ${language === '中文' ? 'bg-[var(--dawn)]' : 'bg-[var(--mercury)]/8'}`}
            style={{ color: language === '中文' ? activeTextColor : inactiveTextColor }}
          >
            中文
          </button>
        </motion.div>
...
export default React.forwardRef<HTMLElement, NavigationProps>(function Navigation({ 
  isDarkMode = false,
  className = "",
  currentSection = "hero"
}, ref) {
  ...
  return (
    <nav 
      ref={ref}
      className={`fixed top-0 left-0 z-50 w-screen ${isDarkMode ? 'nav-dark' : 'nav-light'} ${className}`}
      style={{ 
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" size="sm" className="rounded-none">
                  <Menu className="h-6 w-6 transition-colors duration-300" style={{ color: isDarkMode ? 'var(--midnight)' : 'var(--mercury)' }} />
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-80 bg-[var(--outer-space)] rounded-none border-none [&>button]:text-[var(--mercury)] [&>button]:hover:text-[var(--dawn)] [&>button]:p-4 [&>button]:m-2">
